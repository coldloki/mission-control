...
export async function GET() {
  try {
    ...
    // 2. Get all running node/python processes
    const psOutput = execSync(
      'ps -eo pid,ppid,pcpu,pmem,etime,comm,args | grep -v grep | grep -E -i "node|python|next" | head -100',
    ...
    for (const line of lines) {
      ...
      if (parts[5] === "ps" || parts[6]?.includes("ps -eo") || parts[6]?.includes("grep")) continue;

      const cpu = parseFloat(parts[2]) || 0;
      const memory = parseFloat(parts[3]) || 0;
      const elapsed = parts[4];
      const cmd = parts.slice(6).join(" ").substring(0, 200);
